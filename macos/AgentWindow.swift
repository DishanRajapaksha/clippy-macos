...
final class AgentManagerWindowController: NSWindowController, NSTableViewDataSource, NSTableViewDelegate, NSWindowDelegate {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        switch column {
        case .active:
            let button = sessionButton(type: .radio, sessionID: session.id, action: #selector(activateSession(_:)))
        ...
        case .character:
            ...
            if let configuredName = session.settings.agentName,
               let index = names.firstIndex(where: { $0.caseInsensitiveCompare(configuredName) == .orderedSame }) {
        ...
        case .visible:
            let button = sessionButton(type: .switch, sessionID: session.id, action: #selector(toggleVisibility(_:)))
        ...
        case .muted:
            let button = sessionButton(type: .switch, sessionID: session.id, action: #selector(toggleMute(_:)))
        ...
        case .autoAnimate:
            ...
            addAutoAnimateItem("Random", value: AgentController.randomAutoAnimateInterval, to: popup)
        ...
        case .reactions:
            let button = sessionButton(type: .switch, sessionID: session.id, action: #selector(toggleReactions(_:)))
        ...
        case .position:
            ...
            let label = NSTextField(labelWithString: "\(Int(frame.origin.x)), \(Int(frame.origin.y)) · \(screenName)")
            label.lineBreakMode = .byTruncatingTail
            label.toolTip = "Window origin on \(screenName)"
            return padded(label)

        case .actions:
            let focusButton = AgentSessionButton(title: "Focus", target: self, action: #selector(focusSession(_:)))
            focusButton.sessionID = session.id
            focusButton.controlSize = .small

            let closeButton = AgentSessionButton(title: "Close", target: self, action: #selector(closeSession(_:)))
    ...
    private func configureContent() {
        ...
        let newButton = NSButton(title: "New Random Agent", target: self, action: #selector(newRandomAgent(_:)))
        let showAllButton = NSButton(title: "Show All", target: self, action: #selector(showAll(_:)))
        let hideAllButton = NSButton(title: "Hide All", target: self, action: #selector(hideAll(_:)))

        let spacer = NSView()
        spacer.setContentHuggingPriority(.defaultLow, for: .horizontal)

        let toolbar = NSStackView(views: [countLabel, spacer, newButton, showAllButton, hideAllButton])
        ...
        let footer = NSTextField(labelWithString: "Each row controls one independent agent session. Double-click a row to focus it.")
    ...
    private func sessionButton(type: NSButton.ButtonType, sessionID: UUID, action: Selector) -> AgentSessionButton {
    ...
    private func addAutoAnimateItem(_ title: String, value: TimeInterval, to popup: NSPopUpButton) {
    ...
    @objc private func toggleMute(_ sender: AgentSessionButton) {
        guard let id = sender.sessionID else { return }
        sessionManager?.updateSettings(for: id) { $0.isMuted = sender.state == .on }
        refresh()
    }

    @objc private func toggleReactions(_ sender: AgentSessionButton) {
        guard let id = sender.sessionID else { return }
        sessionManager?.updateSettings(for: id) { $0.proximityReactionsEnabled = sender.state == .on }
        refresh()
    }
    ...
    @objc private func changeAutoAnimate(_ sender: AgentSessionPopUpButton) {
        ...
        sessionManager?.updateSettings(for: id) { $0.autoAnimateInterval = number.doubleValue }
        refresh()
    }
}
